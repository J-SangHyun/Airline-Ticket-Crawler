...
if __name__ == '__main__':
    ...
    for i in range(len(airline_buttons)):
        airline_buttons[i].place(x=10, y=20*i+10)
    ...
    for i in range(n_dates):
        y = (i+1)*30
        dates_from[i].place(x=380, y=y)
        dates_from_t[i].place(x=450, y=y)
        dates_period[i].place(x=500, y=y)
        dates_period_t[i].place(x=520, y=y)

    progress = ttk.Progressbar(main, length=200)
    progress.place(x=380, y=360)
